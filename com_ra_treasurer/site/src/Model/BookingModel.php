...
class BookingModel extends ItemModel
{
	...
	{
		$app  = Factory::getApplication('com_ra_treasurer');
		$user = $app->getIdentity();

		// Check published state
		if ((!$user->authorise('core.edit.state', 'com_ra_treasurer')) && (!$user->authorise('core.edit', 'com_ra_treasurer')))
		{
		...
		{
			$id = Factory::getApplication()->getUserState('com_ra_treasurer.edit.booking.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_ra_treasurer.edit.booking.id', $id);
		}
		...
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_RA_TREASURER_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_RA_TREASURER_ITEM_NOT_LOADED'), 404);
			}
		}
	...
	public function getItemIdByAlias($alias)
	{
		...
		{
			$aliasKey   = $this->getAliasFieldNameByView('booking');
		}
